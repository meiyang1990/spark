...
/**
 * 文件级注释：磁盘块对象写入器，用于将JVM对象直接写入磁盘文件，支持追加写入已有块，
 * 在多次提交之间保留底层文件通道以提升性能，支持原子回滚未提交的部分写入。
 * 不支持并发写入，打开后不能重新打开，主要用于Shuffle阶段溢出数据到磁盘。
 *
...
private[spark] class DiskBlockObjectWriter(
  ...
  /**
   * 手动关闭输出流特质，用于避免重复关闭底层文件通道，仅刷新缓存而不实际关闭底层流，
   * 支持多次提交复用同一个底层文件通道，提升性能。
  ...
  /**
   * 打开写入器，初始化底层资源，准备接收写入，返回当前写入器实例
   */
   ...
   * 提交所有未写入的部分数据，然后关闭资源，OutputStream接口的close方法实现
   */
   ...
   * 刷新未提交的部分写入，将其作为一个原子块提交，返回本次提交对应的文件段信息
   */
   ...
   * 回滚所有未提交的写入，将文件截断到上次提交后的位置，发生异常时调用，不会抛出异常，返回写入的目标文件
   */
   ...
   * 回滚写入指标并删除当前写入的文件，写入过程发生异常且文件不再需要时调用
   */
   ...
   * 更新Shuffle写入指标中的已写入字节数，周期性调用避免频繁更新指标
   */
