...
/**
 * CachedQuantile 对象的Protobuf序列化/反序列化器
 * 用于将Spark阶段任务指标分位数统计数据转换为Protobuf二进制格式进行持久化存储，以及从存储中读取恢复
 */
private[protobuf] class CachedQuantileSerializer extends ProtobufSerDe[CachedQuantile] {

  /**
   * 将CachedQuantile内存对象序列化为Protobuf二进制字节数组
   * @param data 需要序列化的CachedQuantile对象，包含阶段任务指标分位数统计信息
   * @return 序列化后的二进制字节数组
   */
  ...
  /**
   * 将Protobuf二进制字节数组反序列化为CachedQuantile内存对象
   * @param bytes 存储序列化数据的二进制字节数组
   * @return 反序列化得到的CachedQuantile对象
   */
