...
/**
 * Spark内部简化实现Hadoop Configurable接口的特征，封装配置存储逻辑
 */
...
/**
 * 全文件读取的RecordReader实现，将整个文本文件读取为一条记录，key为文件路径，value为文件完整内容
 * 用于Spark wholeTextFiles API，支持将小文件合并读取，减少分区数量提升性能
 */
...
/**
 * 支持配置传播的CombineFileRecordReader实现，将Hadoop配置传递给内部的RecordReader实例
 * 用于配合WholeTextFileRecordReader实现合并小文件的全文件读取
 * @tparam K 记录key类型
 * @tparam V 记录value类型
 */
private[spark] class ConfigurableCombineFileRecordReader[K, V](
  ...
  override def initNextRecordReader(): Boolean = {
    ...
    if (r) {
      // 初始化完成后将当前配置传递给新创建的RecordReader
      if (getConf != null) {
