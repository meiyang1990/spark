...
/**
 * R语言后端连接的身份验证辅助类，适配SparkR与JVM核心通信的身份验证流程
 * 继承SocketAuthHelper，使用R语言特定的字符串序列化格式处理身份验证信息
 */
private[spark] class RAuthHelper(conf: SparkConf) extends SocketAuthHelper(conf) {
  // R语言通信不使用Unix域套接字，固定为网络套接字
  override val isUnixDomainSock = false

  /**
   * 从R连接套接字读取UTF-8格式的身份验证字符串
   * 使用R专属序列化格式读取字符串内容
   * @param s R连接套接字
   * @return 读取到的身份验证字符串
   */
  ...
  /**
   * 向R连接套接字写入UTF-8格式的身份验证字符串
   * 使用R专属序列化格式写入字符串并刷新输出缓冲区
   * @param str 待发送的身份验证字符串
   * @param s R连接套接字
   */
