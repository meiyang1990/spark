...
/**
 * 文件提交协议实现，基于旧版Hadoop MapRed API的底层OutputCommitter提供能力
 * 
 * 与原生Hadoop OutputCommitter不同，本实现支持序列化，可在Spark分布式环境中传输
 *
...
class HadoopMapRedCommitProtocol(jobId: String, path: String)
  ...
  /**
   * 初始化并获取旧版MapRed API的OutputCommitter实例
   * 
   * @param context 新API的任务尝试上下文
   * @return 旧版MapRed API的输出提交器
   */
